...
def extractAndPrintPayload(dataType, payload):
    ...
    if dataType == 9:
        ...
        messageFormPayload.ParseFromString(payload)
        # cv::Mat wrapped(HEIGHT, WIDTH, CV_8UC4, sharedMemory->data());
        # cv2.Mat frame(messageFromPayLoad['height'],messageFromPayLoad['width'],CV_8UC3, messageFromPayLoad['data'])
        # cv2.imshow("output", frame)
        # print("Payload: %s" % (str(messageFromPayload)))

    if dataType == 1037:
        messageFromPayload = opendlv_standard_message_set_v0_9_9_pb2.opendlv_proxy_VoltageReading()
        messageFromPayload.ParseFromString(payload)
        print("Payload: %s" % (str(messageFromPayload)))
