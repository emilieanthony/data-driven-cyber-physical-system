...
cv::Mat calculateSteeringAngelWithColorSpace(cv::Mat inputImage, cv::Mat outputImage, opendlv::proxy::GroundSteeringRequest gsr, auto ms);
cv::Mat calculateSteeringAngleWithYolo(cv::Mat inputImage, cv::Mat outputImage, opendlv::proxy::GroundSteeringRequest gsr, auto ms);
...
int32_t main(int32_t argc, char **argv)
{
    ...
    if ((0 == commandlineArguments.count("cid")) ||
        ...
        (0 == commandlineArguments.count("method")) //add detection methods options
    )
    {
        ...
        std::cerr << "         --method: colorspace or yolo for different detection method" << std::endl;
        std::cerr << "Example: " << argv[0] << " --cid=253 --name=img --width=640 --height=480 --verbose --method=colorspace" << std::endl;
    }
    else
    {
        ...
        const std::string METHOD{commandlineArguments["method"]};
        ...
        if (sharedMemory && sharedMemory->valid())
        {
            ...
            while (od4.isRunning())
            {
                ...
                case "colorspace":
                    /* code */
                    calculateSteeringAngelWithColorSpace(img, cropedImg, gsr, ms);
                ...
                if (fps > 0)
                {
                    ...
                    cv::putText(img, fps_label_str.c_str(), cv::Point(10, 260), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 0, 255), 2);
                }
                // end draw fps
...
cv::Mat calculateSteeringAngelWithColorSpace(cv::Mat inputImage, cv::Mat outputImage, opendlv::proxy::GroundSteeringRequest gsr, auto ms)
{
    ...
    cv::Mat blueThreshImg;
    cv::Mat yellowThreshImg;
    cv::Mat hsvImg;
    //color space
    cv::Mat cropedImg = inputImage(cv::Range(310, 370), cv::Range(0, 640));
    ...
    cv::putText(inputImage,                 // target image
                output,                     // text
